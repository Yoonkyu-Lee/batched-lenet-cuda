...
Deadline: **May 1st, 2026, 8 PM CST**
...
Your final submission must have correct accuracy for any batch size. Therefore, avoid any optimizations that could impact accuracy in your final submission.
...
**We give you a report template: `ECE408_SP26_netid_m3_report.docx`.** Please use this document to get started with your report.
...
- export your report as pdf and name it as `ECE408_SP26_netid_m3_report.pdf`.
...
**Deadline: May 5, 2026, 8:00 PM CST.** The grace period does not apply to the final competition.

By submitting `project/src/layer/custom/m3-forward.cu`, you are automatically enrolled in the final competition if each convolution layer achieves an Op Time below **40ms**. If you do not wish to participate, please contact the course staff.

### Requirements

- The competition will evaluate your m3 final submission in `project/src/layer/custom/m3-forward.cu` only.
- The competition will run `m3` in `--competition` mode with `Batch = 10000`.
- Only submissions that match the baseline (basic GPU convolution) accuracies are eligible for ranking. If your submission produces different accuracies, even if they are higher, it will not be eligible for the competition.
- You **must** use matrix unrolling (either explicit or fused) and the matmul implementation **must** be a tiled implementation.
- No CUDA Stream optimization is allowed in the competition submission.
- All GPU kernel calls must occur within the `conv_forward_gpu()` function. For example, do not attempt to move FP16 conversions to the prolog or epilog to make op times look better if you do intend to use FP16 for competition submission.

### Notes

- You can test with `./m3 --competition`.
- If either convolution layer exceeds **40ms** during the first warm-up check in `--competition` mode, the program will abort immediately to avoid wasting GPU hours.
- We will award extra credits to top performers in this competition. The exact amount hasn't been decided yet, but will be announced soon.

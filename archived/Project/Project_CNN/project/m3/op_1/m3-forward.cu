// op_1: __restrict__ qualifier on kernel pointers.
// Base: kernel-fusion-forward.cu (fused unroll + tiled matmul + implicit permute)
// Change: mark mask/input/output as __restrict__ so the compiler can assume
// non-aliasing and hoist loads / use read-only data cache (ldg) for mask.

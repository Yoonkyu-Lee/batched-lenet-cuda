...
// Surface CUDA failures immediately so silent launch/configuration bugs do not
// masquerade as low accuracy later in the network.
...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    // assign block and thread indices to variables
    // Decode the block mapping as:
    // z -> batch, y -> output height tiles, x -> (map, output width tile).
    // This avoids overflowing grid.z when Batch * Map_out becomes large.
    int tiles_w = (Width_out + blockDim.x - 1) / blockDim.x;
    int b = blockIdx.z;
    int m = blockIdx.x / tiles_w;
    int tile_w = blockIdx.x % tiles_w;
    int h = blockIdx.y * blockDim.y + threadIdx.y;
    int w = tile_w * blockDim.x + threadIdx.x;

    // One thread computes one element of the output feature map
    if (b < Batch && m < Map_out && h < Height_out && w < Width_out) { //check range!!!!!!!
        float acc = 0.0f;
        ...
        out_4d(b, m, h, w) = acc; // assign the computed value to the output feature map
    }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    const int tiles_h = (Height_out + blockDim.y - 1) / blockDim.y;

    // Map_out is folded into grid.x and Batch stays in grid.z so we stay within
    // the CUDA grid.z limit even for large batches on the second conv layer.
    dim3 gridDim(
        tiles_w * Map_out,
        tiles_h,
        Batch
